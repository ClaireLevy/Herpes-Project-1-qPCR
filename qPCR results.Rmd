...
# I want the strip text above the plot to say "186" not "V186"
# so following advice here:http://stackoverflow.com/questions/3472980/ggplot-how-to-change-facet-labels
# Notice that I put the ~ before "Treatment" in my facet_grid call. Otherwise the strips are on the right edge.


virus_names<-c("SD90" = "SD90",
               "V186" = "186")
...
ggplot(epiAnalysis,(aes(x = Time, y = log10(TargetRNA))))+
  ...
  labs(y = "Fold change normalized to ACTB and relative to Mock \n (log10 transformed)", x = "Time (hrs)")+
theme(axis.text = element_text(size = 12),
      axis.title.x = element_text(size=12), 
    axis.title.y = element_text(size=12, vjust = 0.6),
    strip.text = element_text(size =12))+
facet_grid(.~ Treatment , labeller = as_labeller(virus_names))
...
#by donor
ggplot(explantAnalysis,(aes(x = Time, y = FoldChange)))+
...
ggplot(explantAnalysis,(aes(x = Time, y = log10(FoldChange))))+
  ...
  stat_summary(fun.y = mean, geom="point", size = 4.5)+
